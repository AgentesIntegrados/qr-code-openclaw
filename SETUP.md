# QR Code OpenClaw - Setup

## Como funciona

App Next.js que gera QR Code do WhatsApp via `openclaw channels login --channel whatsapp` direto no navegador, sem precisar do terminal.

Ao escanear o QR, a app automaticamente reinicia o gateway do OpenClaw para ativar o canal.
...
PORT=3001
WACLI_STORE=/root/.openclaw/credentials/whatsapp/default
...
- Symlink: `/root/.wacli` -> `/root/.openclaw/credentials/whatsapp/default`
- OpenClaw configurado em `/root/.openclaw/openclaw.json`

## Rodar local (apenas localhost)

```bash
cd /root/qr-code-openclaw
npx next dev -p 3001
```
